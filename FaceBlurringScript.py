...
def pixelnum(width,height,blurrValue):
    Pixel_width=width//blurrValue
    Pixel_height=height//blurrValue
...
for (x,y,w,h) in faces: #faces is a list containing bounding box coordinates and dimensions
    ...
    resolution=pixelnum(faces[boxnum,3],faces[boxnum,2],20) #acquires size of the censoring pixels
    for j in range(faces[boxnum,0],faces[boxnum,0]+faces[boxnum,2],resolution): #iterating through the bounding box 
        for i in range(faces[boxnum,1], faces[boxnum,1]+faces[boxnum,3],resolution):             
            pixelvalue=sum(raw_image[i:i:20,j:j+20])/400  #average of the range 
...
#     resolution=pixelnum(faces[boxnum,3],faces[boxnum,2],20)
#     for j in range(faces[boxnum,0],faces[boxnum,0]+faces[boxnum,2],resolution): #iterating through the bounding box 
#         for i in range(faces[boxnum,1], faces[boxnum,1]+faces[boxnum,3],resolution):
#             pixelvalue=sum()
